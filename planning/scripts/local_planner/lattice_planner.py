    ...
    def run_step(self, ego_state, obstacle_detections):
        ...
        if self.best_path is not None:            
            for ind in range(len(self.best_path.x)):
                state = State()
                state.x = self.best_path.x[ind]
                state.y = self.best_path.y[ind]
                state.yaw = self.best_path.yaw[ind]
                state.velocity = math.hypot(self.best_path.s_d[ind], self.best_path.d_d[ind])
