...
class ControllerBridge:
    ...
    def _teleop_callback(self, msg):
        if DEBUG_MSGS:
            rospy.loginfo("ControllerBridge._teleop_callback")
        ...
        if DEBUG_MSGS:
            rospy.loginfo("ControllerBridge._teleop_callback done")
    
    def apply_control(self, ctrl=None):
        if DEBUG_MSGS:
            rospy.loginfo("ControllerBridge.apply_control")
        if ctrl is not None:
            if DEBUG_MSGS:
                rospy.loginfo("ControllerBridge.apply_control ctrl: " + str(ctrl))
            self._vehicle.apply_control(ctrl)
        else:
            if DEBUG_MSGS:
                rospy.loginfo("ControllerBridge.apply_control self._control: " + str(self._control))
            # got stuck at the following line once
            self._vehicle.apply_control(self._control)   
        if DEBUG_MSGS:
            rospy.loginfo("ControllerBridge.apply_control done")
        
    def set_control(self, throttle, steering):
        if DEBUG_MSGS:
            rospy.loginfo("ControllerBridge.set_control")
        ...
        if DEBUG_MSGS:
            rospy.loginfo("ControllerBridge.set_control done")
