...
class open3d_vis_utils:
    ...
    def draw_scenes(vis, points, gt_boxes=None, ref_boxes=None, ref_labels=None, ref_scores=None, point_colors=None, draw_origin=True, confidence=None, tracks=None):
        ...
        if gt_boxes is not None:
            vis = open3d_vis_utils.draw_box(vis, gt_boxes, (0, 0, 1.0))

        if ref_boxes is not None:
            vis = open3d_vis_utils.draw_box(vis, ref_boxes, (0, 1.0, 0), ref_labels, ref_scores, confidence)
            
        if tracks is not None:
            vis = open3d_vis_utils.draw_tracks(vis, tracks, (0, 1.0, 0), ref_labels)
    ...
    def draw_tracks(vis, tracks, color=(0, 1.0, 0), ref_labels=None):
        for i in range(tracks.__len__()):
            ...
            if ref_labels is None:
                ...
                uni_color = open3d_vis_utils.box_colormap[ref_labels[i]]
    ...
    def draw_box(vis, gt_boxes, color=(0, 1.0, 0), ref_labels=None, score=None, confidence=None):
        for i in range(gt_boxes.shape[0]):
            ...
            line_set, box3d = open3d_vis_utils.translate_boxes_to_open3d_instance(gt_boxes[i])

            if ref_labels is None:
                line_set.paint_uniform_color(color)
            else:
                line_set.paint_uniform_color(open3d_vis_utils.box_colormap[ref_labels[i]])
